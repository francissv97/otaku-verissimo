...
export function AnimeHeader() {
  ...
  /* TODO: Implementar autenticação com AniList e integração com o GraphQL para obter os dados do usuário no header */
  return (
    <div className="fixed left-0 right-0 top-0 z-20 w-full">
      <div className="mx-auto flex w-full max-w-5xl justify-between p-4">
        ...
        {user && (
          <div
            title={user.name}
            className="flex h-14 w-14 cursor-pointer items-center justify-center self-center rounded-full transition hover:bg-white/40"
            onClick={() => navigate("/profile")}
          >
            <img src={user.avatar.medium} className="w-12" />
          </div>
        )}
      </div>
    </div>
