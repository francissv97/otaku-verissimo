...
export function CardAnimeResult({ anime }: CardAnimeResultProps) {
  ...
  return (
    <HoverCard.Root key={anime.id} openDelay={0} closeDelay={0}>
      <HoverCard.Trigger className="relative" asChild>
        <Link to={`/anime/${anime.id}`}>
          <div className="group flex h-full cursor-pointer flex-col">
            <div className="relative mb-3 overflow-hidden rounded-lg bg-gradient-to-t from-orange-700 via-orange-600 to-orange-500 shadow-md">
              <img
                src={anime.coverImage.large}
                alt={anime.title.romaji}
                className="aspect-[6_/_9] h-full w-full object-cover object-center group-hover:border-main"
                loading="lazy"
                style={{
                  opacity: 0,
              ...
              <div className="absolute bottom-0 h-0 w-full bg-gradient-to-t from-main/50 via-transparent to-transparent duration-200 group-hover:h-full"></div>
            </div>
      ...
      {screenSizeMatches && (
        <HoverCard.Portal>
          <HoverCard.Content className="pointer-events-none" side="right" align="start" sideOffset={14}>
